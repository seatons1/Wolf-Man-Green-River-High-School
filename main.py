...
# TOUCH CONTROLS
# ============================================================
# On-screen controls make the game playable on phones/tablets.
# They also respond to simple swipes anywhere on the playfield.
TOUCH_BUTTON_SIZE = 54
TOUCH_BUTTON_GAP = 7
TOUCH_MARGIN = 16
TOUCH_PAD_X = WIDTH - TOUCH_MARGIN - 3 * TOUCH_BUTTON_SIZE - 2 * TOUCH_BUTTON_GAP
TOUCH_PAD_Y = HEIGHT - TOUCH_MARGIN - 2 * TOUCH_BUTTON_SIZE - TOUCH_BUTTON_GAP
TOUCH_BUTTONS = {
    "up": pygame.Rect(
        TOUCH_PAD_X + TOUCH_BUTTON_SIZE + TOUCH_BUTTON_GAP,
        TOUCH_PAD_Y, TOUCH_BUTTON_SIZE, TOUCH_BUTTON_SIZE
    ),
    "left": pygame.Rect(
        TOUCH_PAD_X,
        TOUCH_PAD_Y + TOUCH_BUTTON_SIZE + TOUCH_BUTTON_GAP,
    ...
    "right": pygame.Rect(
        TOUCH_PAD_X + 2 * (TOUCH_BUTTON_SIZE + TOUCH_BUTTON_GAP),
        TOUCH_PAD_Y + TOUCH_BUTTON_SIZE + TOUCH_BUTTON_GAP,
        TOUCH_BUTTON_SIZE, TOUCH_BUTTON_SIZE
    ),
}
...
def draw_touch_controls():
    """Draw a compact translucent D-pad for touch devices."""
    overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    panel = pygame.Rect(TOUCH_PAD_X - 8, TOUCH_PAD_Y - 8,
                       3 * TOUCH_BUTTON_SIZE + 2 * TOUCH_BUTTON_GAP + 16,
                       2 * TOUCH_BUTTON_SIZE + TOUCH_BUTTON_GAP + 16)
    pygame.draw.rect(overlay, (0, 20, 10, 115), panel, border_radius=18)

    labels = {"up": "▲", "left": "◀", "down": "▼", "right": "▶"}
    for name, rect in TOUCH_BUTTONS.items():
        pygame.draw.rect(overlay, (20, 115, 55, 190), rect, border_radius=12)
        pygame.draw.rect(overlay, (100, 245, 135, 230), rect, 2, border_radius=12)
        text = subtitle_font.render(labels[name], True, WHITE)
        overlay.blit(text, text.get_rect(center=rect.center))

    hint = small_font.render("TOUCH / SWIPE TO STEER", True, WHITE)
    hint_rect = hint.get_rect(right=panel.right, bottom=panel.top - 5)
    overlay.blit(hint, hint_rect)
    screen.blit(overlay, (0, 0))


def touch_direction_at(pos):
    """Return a direction if a touch/click lands on a D-pad button."""
    for name, rect in TOUCH_BUTTONS.items():
        if rect.collidepoint(pos):
            return TOUCH_DIRECTION_MAP[name]
    return None


def handle_swipe(finger_id, x, y):
    """Convert a finger movement into one of the four directions."""
    if finger_id not in TOUCH_SWIPES:
        return None
    start_x, start_y = TOUCH_SWIPES[finger_id]
    dx = x - start_x
    dy = y - start_y
...
def draw_maze():
    """Draw the upgraded Green River arcade-style playing field."""
    # Deep green arcade background.
    screen.fill((3, 28, 16))

    # Subtle background grid keeps the playfield lively without distracting
    # from the maze.
    for gx in range(0, WIDTH, 16):
        pygame.draw.line(screen, (5, 43, 23), (gx, HUD_HEIGHT), (gx, HEIGHT), 1)
    for gy in range(HUD_HEIGHT, HEIGHT, 16):
        pygame.draw.line(screen, (5, 43, 23), (0, gy), (WIDTH, gy), 1)

    # HUD panel with a bright Green River accent line.
    pygame.draw.rect(screen, (4, 42, 23), (0, 0, WIDTH, HUD_HEIGHT))
    pygame.draw.rect(screen, GREEN, (0, HUD_HEIGHT - 3, WIDTH, 3))

    # Small GR badge.
    badge = pygame.Rect(10, 9, 44, 48)
    pygame.draw.rect(screen, (7, 67, 35), badge, border_radius=9)
    pygame.draw.rect(screen, LIGHT_BLUE, badge, 2, border_radius=9)
    gr_text = small_font.render("GR", True, WHITE)
    screen.blit(
        gr_text,
        (
            badge.centerx - gr_text.get_width() // 2,
            badge.centery - gr_text.get_height() // 2,
        ),
    )

    score_text = font.render(f"SCORE: {score}", True, WHITE)
    high_text = small_font.render(f"BEST: {high_score}", True, LIGHT_BLUE)
    ...
    if wolf_character_image is not None:
        icon = pygame.transform.smoothscale(wolf_character_image, (23, 23))
        for i in range(lives):
            screen.blit(icon, (WIDTH - 27 - i * 27, 39))

    if invulnerable_timer > 0:
        safe_text = small_font.render("WOLF RECOVERING!", True, CYAN)
        screen.blit(
            safe_text,
            (WIDTH // 2 - safe_text.get_width() // 2, 42),
        )

    # Maze floor: give each open tile a subtle green inset.
    for row in range(ROWS):
        for col in range(COLS):
            if grid[row][col] != "#":
                rect = pygame.Rect(
                ...
                pygame.draw.rect(screen, (3, 34, 18), rect)
                pygame.draw.rect(
                    screen,
                    (5, 50, 25),
                    rect.inflate(-2, -2),
    ...
    # Walls: layered neon-green blocks with rounded corners and highlights.
    for row in range(ROWS):
        for col in range(COLS):
            if grid[row][col] == "#":
                rect = pygame.Rect(
                ...
                # Tiny highlight along the top-left edge.
                pygame.draw.line(
                    screen,
                    (76, 220, 112),
                    (inner.left + 5, inner.top + 3),
                    (inner.right - 5, inner.top + 3),
                    1,
                )

    # Pellets: small glowing gold dots.
    for row in range(ROWS):
        for col in range(COLS):
            x, y = tile_center(col, row)

            if grid[row][col] == ".":
                pygame.draw.circle(screen, (115, 91, 18), (x, y), 5)
                ...
                pulse = (
                    8
                    + int(
                        2 * math.sin(pygame.time.get_ticks() * 0.008)
                    )
                )

                # Soft halo.
                halo = pygame.Surface((pulse * 4, pulse * 4), pygame.SRCALPHA)
                pygame.draw.circle(
                    halo,
                    (255, 220, 40, 35),
                    (pulse * 2, pulse * 2),
                ...
                pygame.draw.circle(screen, (150, 110, 10), (x, y), pulse + 2)
                pygame.draw.circle(screen, YELLOW, (x, y), pulse)
                pygame.draw.circle(screen, WHITE, (x - 2, y - 2), 2)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if game_state == "title":
                ...
                direction = touch_direction_at(event.pos)
                if direction is not None:
                    wolf.set_direction(direction)
            ...
            # Pygame finger coordinates are normalized from 0.0 to 1.0.
            px = int(event.x * WIDTH)
            py = int(event.y * HEIGHT)
            TOUCH_SWIPES[event.finger_id] = (px, py)
            init_audio()

            if game_state == "title":
                start_from_user_action()
            elif game_state == "playing":
                direction = touch_direction_at((px, py))
                if direction is not None:
                    wolf.set_direction(direction)
            ...
            px = int(event.x * WIDTH)
            py = int(event.y * HEIGHT)
            direction = handle_swipe(event.finger_id, px, py)
            if game_state == "playing" and direction is not None:
                wolf.set_direction(direction)
                TOUCH_SWIPES[event.finger_id] = (px, py)

        elif event.type == pygame.FINGERUP:
            TOUCH_SWIPES.pop(event.finger_id, None)
...
async def main():
    ...
    while running:
        ...
        if game_state == "title":
            ...
            # Mobile-friendly on-screen controls. Keyboard controls remain unchanged.
            draw_touch_controls()
